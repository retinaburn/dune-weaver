...
class TelnetClient:
    ...
    def read_until(self, separator=b'\n'):
        async def myread():
            ...
            try:
                readdata = await self.reader.readuntil(separator)
                #readdata = await self.reader.read()
    ...
    def read(self):
        async def myread():
            try:
                readdata = await self.reader.read(100)
                logger.debug(f"Read data: {readdata}")
                return readdata.encode()
            except Exception as e:
                logger.error(f"Error in myread: {e}")
                return None
        return self.run_coroutine(myread())
    ...
    def write(self, data):
        async def mywrite():
            try:
                ...
                return False
        return self.run_coroutine(mywrite())
    ...
    def is_connected(self):
        logger.debug(f"is_connected: reader:{client.reader.connection_closed} writer:{client.writer.connection_closed}")
        return  not self.reader.connection_closed and not self.writer.connection_closed
    
    def get_current_status(self):
        with telnet_lock:
            logger.debug("Getting status...")
            self.write("$T\n")
            response = self.read_until("ok\r\n".encode()).strip()
...
def send_grbl_coordinates(x, y, speed=600, timeout=2, home=False):
    ...
    """
    logger.debug(f"Sending G-code: X{x} Y{y} at F{speed}, home={home}")
    while True:
        with telnet_lock:
            if home:
            ...
            client.write(f"{gcode}\n")
            logger.debug(f"Sent command: {gcode}")

            #TODO timeouts dont work?
            start_time = time.time()
            while time.time() - start_time < timeout:
                #TODO read eager
                logger.debug(f"Prepared to read...")
                response = client.read_until().strip()
...
def home(retry=0):
    ...
    try:
        ...
        with telnet_lock:
            logger.debug("Checking for sensorless homing...")
            client.write("$config\n")
            response = client.read_until("ok\r\n".encode()).strip()
            logger.debug(f"Config response: {response}")
    except Exception as e:
        logger.error(f"Error checking for sensorless homing: {e}")
        raise e
    ...
    # we check that we actually got a valid response, if not, we try again a couple of times
        ...
        # we wait for the table to be idle
        while True:
...
def check_idle():
    """
    Just check if its currently idle
    """
    logger.info("Checking idle")
    response = get_status_response()
    if response:
        update_machine_position(response) # might as well update the position since we got it
...
def wait_for_idle():
    ...
    while True:
        response = get_status_response()
        if response:
            update_machine_position(response)
        if response and "Idle" in response:
            logger.info("Table is idle")
            return True  # Exit once 'Idle' is confirmed
        time.sleep(0.1)
...
def get_machine_position(timeout=3):
    ...
    """
    logger.debug("Getting machine position...")
    #TODO timeouts dont work?
    # start_time = time.time()
    # while time.time() - start_time < timeout:
    with telnet_lock:
...
def update_machine_position(response=None):
    logger.info("Updating machine position")
    if response:
        if "MPos" in response:
            pos = parse_machine_position(response)
            if pos:
                machine_x, machine_y = pos
                logger.debug(f"Machine position: X={machine_x}, Y={machine_y}")
                state.machine_x, state.machine_y =  machine_x, machine_y
    else:
